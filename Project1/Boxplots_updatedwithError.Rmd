...
df3 <- eventReactive(c(input$selectCountry_3, input$selectValues_3, input$selectYear_3), {
  ...
  paramQuery <- data.world::qry_sql(
    paste(
      "
  	    select country_name, values, year
        from cleaned_data c
        where cast(year as integer) between
...
```{r}
inputPanel(
  selectInput("selectCountry_3", label = "Select Country",choices = countries_3, multiple=TRUE, selected=c("Mali", "Libya", "Lebanon","Kyrgyz Republic","Latvia","Lesotho","Liberia","Luxembourg","Madagascar","Malawi","Malysia","Malta","Maurianai","Mexico","Mexico","Mongolia","Morocco","Mozambique")),

  sliderInput("selectYear_3", label = "Year:", min = 1980, max = 2014, value = c(2006), step = 1)
)
```
...
tabsetPanel(
  ...
  tabPanel("Plot Title",
    textInput("title_3", "Title", "Index Values"),
    numericInput("titleFont_3", "Title Font", 10)),
  tabPanel("Plot Legend",
    numericInput("legendTitleSize_3", "Legend Title Size", 10),
    numericInput("legendItemSize_3", "Legend Item Size", 10)),
  tabPanel("Axis Labels",
    textInput("xLabel_3", "x-Axis Label", "Years"),
    textInput("yLabel_3", "y-Axis Label", "Values"),
    numericInput("textFont_3", "textFont:", 10)),
  tabPanel("Data Size", 
    numericInput("yDataMin_3", "yData Minimum (Required):", NA, min=0, step = 0.05),
    numericInput("yDataMax_3", "yData Maximum (Required):", NA, min=0, step = 0.05)))
```
...
renderPlotly({
  plot = df4() %>% ggplot() +
    geom_boxplot(mapping = aes(x = country, y = values)) +
    ...
    ggtitle(paste(Countries_3(), input$title_3)) +
